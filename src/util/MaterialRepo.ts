import { Color, DoubleSide, LineBasicMaterial, MeshLambertMaterial, Plane, Vector3 } from 'three';
...
export class MaterialRepo {
  ...
  private static MATERIALS_FACE: { [k in string]: MeshLambertMaterial } = {};
  ...
  static getMaterialFace(colorDesc: IColorDescription): MeshLambertMaterial {
    ...
    if (!this.MATERIALS_FACE[code]) {
      ...
      MaterialRepo.MATERIALS_FACE[code] = new MeshLambertMaterial({
        color: new Color(colorDesc.rgb),
        opacity: colorDesc.opacity,
        transparent: true,
        // roughness: 0.75,
